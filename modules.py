...
def gru(inputs, num_layers=None, num_units=None, direction='bidirectional', dropout=None, seqlens=None, scope="gru", reuse=None, use_gpu=True):
    '''Applies a GRU.
        ...
        if use_gpu == True:
            ...
            for layer in range(num_layers):
                if direction == "bidirectional":
                    ...
                    gru_layer = GRU(units=num_units, dropout=dropout, return_sequences=True)
...
def cbhg(input, num_banks, hidden_units, num_highway_blocks, num_gru_layers, dropout_rate=None, norm_type='bn', is_training=True, scope="cbhg", use_gpu=True):
    with tf.variable_scope(scope):
        ...
        out = gru(out, num_gru_layers, hidden_units, 'bidirectional', dropout_rate,  use_gpu=use_gpu)  # (N, T, E)
    return out
